...
export class S3GoalCacheArchiveStore implements GoalCacheArchiveStore {
    public async store(gi: GoalInvocation, classifier: string, archivePath: string): Promise<void> {
        ...
        await this.awsS3(gi, classifier,
            async (storage, bucket, cachePath) => storage.putObject({Bucket: bucket, Key: cachePath, Body: file}).promise(), "store");
    }

    public async delete(gi: GoalInvocation, classifier: string): Promise<void> {
        await this.awsS3(gi, classifier, async (storage, bucket, cachePath) => storage.deleteObject({Bucket: bucket, Key: cachePath}).promise(), "delete");
    }
